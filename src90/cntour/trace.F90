...
!     ymax=0.5
      deltax=x2-x1
      deltay=y2-y1
      xmin=real((x1+x2)/2.-max(deltax,deltay)/2.,Single)
      xmax=real((x1+x2)/2.+max(deltax,deltay)/2.,Single)
      ymin=real((y1+y2)/2.-max(deltax,deltay)/2.,Single)
      ymax=real((y1+y2)/2.+max(deltax,deltay)/2.,Single)
!***
!     xmin=0.6
!     xmax=0.8
!     ymin=0.3
...
!        do 56 j=1, ny
!
!           x(i)=real(cordx(i),Single)
!           y(j)=real(cordy(j),Single)
...
!  58    continue
      ...
      DO 60 i=1, nreg
         DO 70 j=1, npr(i)
            DO 80 k=1, np1(i)
               ...
               x(k)=real(xmail(k,j,i),Single)
               y(k)=real(ymail(k,j,i),Single)
   ...
   60 CONTINUE
      do 90 i=1,nreg
        do 88 k=1,np1(i)
          do 86 j=1,npr(i)
            x(j)=real(xmail(k,j,i),Single)
            y(j)=real(ymail(k,j,i),Single)
86        continue
